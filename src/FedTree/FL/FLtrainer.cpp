...
void FLtrainer::vertical_fl_dp_trainer(vector<Party> &parties, Server &server, FLParam &params) {

//    initialise help functions
    GBDTParam &model_param = params.gbdt_param;
    Comm comm_helper;
    DifferentialPrivacy dp_manager = DifferentialPrivacy();
    dp_manager.init(params);
    Partition pt_manager;

    std::chrono::high_resolution_clock timer;
    auto start = timer.now();

    for(int round = 0; round < model_param.n_trees; round ++) {
        vector<Tree> trees(dp_manager.ensemble_in_ensemble * model_param.n_trees);

//        data partitions for subensembles
        vector<vector<DataSet>> data_partitions(parties.size());
        std::map<int, vector<int>> batch_idxs;

        for(int i = 0; i < parties.size(); i ++) {
            vector<DataSet> vec(dp_manager.ensemble_in_ensemble);
            data_partitions[i] = vec;
        }

        for(int i = 0; i < parties.size(); i ++) {
            pt_manager.homo_partition(parties[i].dataset, dp_manager.ensemble_in_ensemble, true, data_partitions[i], batch_idxs);
        }

//        build a tree
        for(int t = 0; t < dp_manager.ensemble_in_ensemble; t ++) {
...
//          send gradients to all parties
            for(auto & party : parties) {
                server.send_booster_gradients(party);
            }

//            change the dataset
            for(int pid = 0; pid < parties.size(); pid ++) {
                parties[pid].dataset = data_partitions[pid][t];
            }

            Tree &tree = trees[t + dp_manager.ensemble_in_ensemble * round];
...
//            for each level
            for(int l = 0; l < model_param.depth; l ++) {
//                initialise level params
                int n_nodes_in_level = 1 << l;
                int n_max_nodes = 2 << model_param.depth;
                vector<int> parties_n_bins(parties.size());
                vector<int> parties_n_columns(parties.size());
...
//                each party compute histogram, send to server
                for(int pid = 0; pid < parties.size(); pid ++) {
                    parties_n_columns[pid] = parties[pid].dataset.n_features();
                }
